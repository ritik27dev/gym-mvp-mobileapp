...
//       .slice(0, 1);
//     last7Days.push({ date: localDate, label });
//   }

//   console.log(`Processing ${type}:`, { last7Days, dataLength: data.length });

//   const result = last7Days.map(({ date, label }) => {
...
//   // Create a sparse dataset - only include points where we have actual data
//   const sparseData = [];
//   const sparseLabels = [];

//   result.forEach((item, index) => {
//     if (item.hasData) {
//       sparseData.push(item.value);
//       sparseLabels.push(item.label);
//     }
//   });

//   const finalResult = {
//     labels: result.map((r) => r.label), // Keep all labels for x-axis
//     values: result.map((r) => {
//       // For missing data, use null which should be handled better by the chart
//       return typeof r.value === "number" ? r.value : null;
//     }),
//     sparseData, // Only actual data points
//     sparseLabels, // Only labels with data
//   };
...
//           hidePointsAtIndex={daily.values
//             .map((v, i) => (v === null ? i : -1))
//             .filter((i) => i !== -1)}
//         />
...
const getMoodSeries = (data, type) => {
  ...
  // Create chart data with proper handling of missing values
  // For react-native-chart-kit, we need to ensure we don't have zero values
